    ...
    GIVEN("square with hole") {
        ...
        WHEN("Clipping line 1") {
            ...
            THEN("line is clipped to square with hole") {
                REQUIRE((Vec2f(15, 18) - Vec2f(15, 16)).norm() == Catch::Approx(intersection.front().length()));
            }
        }
        WHEN("Clipping line 2") {
            Polylines intersection = intersection_pl({ Polyline { { 15, 15 }, { 15, 12 } } }, expolygons);
            THEN("line is clipped to square with hole") {
                REQUIRE((Vec2f(15, 14) - Vec2f(15, 12)).norm() == Catch::Approx(intersection.front().length()));
            }
        }
        WHEN("Clipping line 3") {
            Polylines intersection = intersection_pl({ Polyline { { 12, 18 }, { 18, 18 } } }, expolygons);
            THEN("line is clipped to square with hole") {
                REQUIRE((Vec2f(18, 18) - Vec2f(12, 18)).norm() == Catch::Approx(intersection.front().length()));
            }
        }
        WHEN("Clipping line 4") {
            Polylines intersection = intersection_pl({ Polyline { { 5, 15 }, { 30, 15 } } }, expolygons);
            THEN("line is clipped to square with hole") {
                REQUIRE((Vec2f(14, 15) - Vec2f(10, 15)).norm() == Catch::Approx(intersection.front().length()));
                REQUIRE((Vec2f(20, 15) - Vec2f(16, 15)).norm() == Catch::Approx(intersection[1].length()));
            }
        }
        WHEN("Clipping line 5") {
            ...
            THEN("reverse line is clipped to square with hole") {
                REQUIRE((Vec2f(20, 15) - Vec2f(16, 15)).norm() == Catch::Approx(intersection.front().length()));
                REQUIRE((Vec2f(14, 15) - Vec2f(10, 15)).norm() == Catch::Approx(intersection[1].length()));
            }
        }
        WHEN("Clipping line 6") {
            ...
            THEN("tangent line is clipped to square with hole") {
                REQUIRE((Vec2f(20, 18) - Vec2f(10, 18)).norm() == Catch::Approx(intersection.front().length()));
            }
        }
    }
    ...
    GIVEN("circle") {
        ...
        WHEN("clipping a line") {
            ...
            THEN("clipped to two pieces") {
                REQUIRE(intersection.front().length() == Catch::Approx((Vec2d(152742000, 215178843) - Vec2d(152742000, 288086661)).norm()));
                REQUIRE(intersection[1].length() == Catch::Approx((Vec2d(152742000, 35166477) - Vec2d(152742000, 108087507)).norm()));
            }
        }
    }
    GIVEN("line") {
