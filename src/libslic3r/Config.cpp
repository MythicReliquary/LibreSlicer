...
std::vector<std::string> ConfigOptionDef::cli_args(const std::string &key) const
{
	...
	if (this->cli != ConfigOptionDef::nocli) {
        ...
        // Neither '=' nor '!' is used in any of the cli parameters currently defined by LibreSlicer.
...
bool ConfigBase::set_deserialize_raw(const t_config_option_key &opt_key_src, const std::string &value, ConfigSubstitutionContext& substitutions_ctxt, bool append)
{
    ...
    if (optdef->type == coBools && substitutions_ctxt.rule != ForwardCompatibilitySubstitutionRule::Disable) {
    	//FIXME Special handling of vectors of bools, quick and not so dirty solution before LibreSlicer 2.3.2 release.
...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Also the "; generated by LibreSlicer ..." comment line will be removed.
    size_t j = 0;
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &filename, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or LibreSlicer header.
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char prusaslicer_gcode_header[] = "; generated by LibreSlicer ";
        std::string header;
        ...
        if (! header_found)
            throw Slic3r::RuntimeError("Not a LibreSlicer / Slic3r PE generated g-code.");
    }
    ...
    if (has_delimiters)
    {
        // LibreSlicer starting with 2.4.0-alpha0 delimits the config section stored into G-code with 
    ...
    {
        // Slic3r or LibreSlicer older than 2.4.0-alpha0 do not emit any delimiter.
