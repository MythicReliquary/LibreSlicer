...
// For starting another LibreSlicer instance on OSX.
...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance, bool delete_after_load)
{
	...
	// Don't call with wxEXEC_HIDE_CONSOLE, LibreSlicer in GUI mode would just show the splash screen. It would not open the main window though, it would
...
#if defined(__APPLE__)
	{
		...
		// ((instance_type == NewSlicerInstanceType::Slicer) ? "LibreSlicer" : "PrusaGCodeViewer");
		// Just run LibreSlicer and give it a --gcodeviewer parameter.
		bin_path = bin_path.parent_path() / "LibreSlicer";
