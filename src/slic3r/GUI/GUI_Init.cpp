...
{
#if __APPLE__
    // On OSX, we use boost::process::spawn() to launch new instances of LibreSlicer from another LibreSlicer.
    // boost::process::spawn() sets SIGCHLD to SIGIGN for the child process, thus if a child LibreSlicer spawns another
    // subprocess and the subrocess dies, the child LibreSlicer will not receive information on end of subprocess
    // (posix waitpid() call will always fail).
    // https://jmmv.dev/2008/10/boostprocess-and-sigchld.html
    // The child instance of LibreSlicer has to reset SIGCHLD to its default, so that posix waitpid() and similar continue to work.
    // See GH issue #5507
    ...
    try {
        ...
        wxMessageBox(boost::nowide::widen(ex.what()), _L("LibreSlicer GUI initialization failed"), wxICON_STOP);
    } catch (const std::exception &ex) {
        boost::nowide::cerr << "LibreSlicer GUI initialization failed: " << ex.what() << std::endl;
        wxMessageBox(format_wxstr(_L("Fatal error, exception catched: %1%"), ex.what()), _L("LibreSlicer GUI initialization failed"), wxICON_STOP);
    }
