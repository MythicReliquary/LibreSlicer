...
public:
	...
	// returns true if path to LibreSlicer.desktop is stored in App Config and existence of desktop file. 
	...
	// Creates desktop files and icons for both LibreSlicer and GcodeViewer.
	// Stores paths into App Config.
	// Rewrites if files already existed.
	// if perform_downloader:
    // Creates desktop files for LibreSlicer downloader feature
	// Registers LibreSlicer to start on libreslicer:// URL
	static void perform_desktop_integration();
	// Deletes desktop files and icons for both LibreSlicer and GcodeViewer at paths stored in App Config.
