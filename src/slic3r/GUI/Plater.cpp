...
//    p->scrolled->SetScrollbars(0, 100, 1, 2); // ys_DELETE_after_testing. pixelsPerUnitY = 100 from https://github.com/prusa3d/LibreSlicer/commit/8f019e5fa992eac2c9a1e84311c990a943f80b01, 
...
void Sidebar::show_info_sizer()
{
    ...
    if (selection.is_single_volume()) {
        ...
        if (vol_idxs.size() != 1)
            ...
            // see https://github.com/prusa3d/LibreSlicer/issues/7408
            return;
                ...
                if (act.empty()) {
                    ...
                    if (res != wxID_CANCEL)
                        if (dialog.IsCheckBoxChecked()) {
                            ...
                            MessageDialog msg_dlg(mainframe, msg, _L("LibreSlicer: Don't ask me again"), wxOK | wxCANCEL | wxICON_INFORMATION);
...
void Plater::notify_about_installed_presets()
{
    ...
    if (!names.empty()) {
        std::string notif_text = into_u8(_L_PLURAL("The preset below was temporarily installed on the active instance of LibreSlicer",
            "The presets below were temporarily installed on the active instance of LibreSlicer", names.size())) + ":";
                ...
                if (imperial_units)
                    ...
                    if (answer_convert_from_meters == wxOK_DEFAULT) {
                        RichMessageDialog dlg(q, format_wxstr(_L_PLURAL(
                            ...
                            "The internal unit of LibreSlicer is a millimeter. Do you want to recalculate the dimensions of the object?",
                            "The dimensions of some objects from file %s seem to be defined in meters.\n"
                            "The internal unit of LibreSlicer is a millimeter. Do you want to recalculate the dimensions of these objects?", model.objects.size()), from_path(filename)) + "\n",
    ...
    if (obj->is_cut()) {
        InfoDialog dialog(q, _L("Delete object which is a part of cut object"), 
                             _L("You try to delete an object which is a part of a cut object.") + "\n" + 
                                ...
                                "After that LibreSlicer can't guarantee model consistency"), 
...
void Plater::priv::on_select_preset(wxCommandEvent &evt)
{
    ...
    // see https://github.com/prusa3d/LibreSlicer/issues/3889
    ...
    // (see https://github.com/prusa3d/LibreSlicer/issues/5531 ).
    ...
    if (evt.status.percent >= 100 && this->printer_technology == ptFFF) {
        ...
        if (templ_cnt > 0) {
            ...
            const std::string message_dial = GUI::format("%1% %2% %3%"
                , _L_PLURAL("You are using template filament preset.", "You are using template filament presets.", templ_cnt)
                , _u8L("Please note that template presets are not customized for specific printer and should only be used as a starting point for creating your own user presets.")
                , "<a href=https://github.com/MythicReliquary/LibreSlicer-Supporter-Dev/wiki/Template-Filaments>LibreSlicer documentation</a>"
            );
            BOOST_LOG_TRIVIAL(warning) << message_notif;
            notification_manager->push_slicing_warning_notification(message_notif, false, 0, 0, "https://github.com/MythicReliquary/LibreSlicer-Supporter-Dev/wiki/Template-Filaments", 
                [](wxEvtHandler* evnthndlr) { wxGetApp().open_browser_with_warning_dialog("https://github.com/MythicReliquary/LibreSlicer-Supporter-Dev/wiki/Template-Filaments"); return false; }
                );
        ...
        wxDefaultSize, wxDEFAULT_DIALOG_STYLE)
{
    ...
    if (instances_allowed) {
        btn = new wxRadioButton(this, wxID_ANY, _L("Start a new instance of LibreSlicer"), wxDefaultPosition, wxDefaultSize, id == 0 ? wxRB_GROUP : 0);
        btn->SetValue(id == m_action);
        btn->Bind(wxEVT_RADIOBUTTON, [this, id, contains_projects](wxCommandEvent&) {
            m_action = id;
            if (contains_projects) {
        ...
        wxDefaultSize, wxDEFAULT_DIALOG_STYLE)
{
    ...
    if (!single_instance_only)
        choices.Add(_L("Start new LibreSlicer instance"));
...
void Plater::send_gcode()
{
    ...
    if (dlg.ShowModal() == wxID_OK) {
        ...
        // Show "Is printer clean" dialog for LibreConnect - Upload and print.
        if (std::string(upload_job.printhost->get_name()) == "LibreConnect" && upload_job.upload_data.post_action == PrintHostPostUploadAction::StartPrint) {
