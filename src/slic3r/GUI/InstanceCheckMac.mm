...
-(void)add_observer:(NSString *)version_hash
{
	...
	//NSString *nsver = @"OtherLibreSlicerInstanceMessage" + version_hash;
	NSString *nsver = [NSString stringWithFormat: @"%@%@", @"OtherLibreSlicerInstanceMessage", version_hash];
	...
	NSString *nsver2 = [NSString stringWithFormat: @"%@%@", @"OtherLibreSlicerInstanceClosing", version_hash];
...
void send_message_mac(const std::string &msg, const std::string &version)
{
	...
	//NSString *nsver = @"OtherLibreSlicerInstanceMessage" + [NSString stringWithCString:version.c_str() encoding:[NSString defaultCStringEncoding]];
	NSString *nsver = [NSString stringWithCString:version.c_str() encoding:[NSString defaultCStringEncoding]];
	NSString *notifname = [NSString stringWithFormat: @"%@%@", @"OtherLibreSlicerInstanceMessage", nsver];
...
void send_message_mac_closing(const std::string &msg, const std::string &version)
{
	...
	//NSString *nsver = @"OtherLibreSlicerInstanceMessage" + [NSString stringWithCString:version.c_str() encoding:[NSString defaultCStringEncoding]];
	NSString *nsver = [NSString stringWithCString:version.c_str() encoding:[NSString defaultCStringEncoding]];
	NSString *notifname = [NSString stringWithFormat: @"%@%@", @"OtherLibreSlicerInstanceClosing", nsver];
	[[NSDistributedNotificationCenter defaultCenter] postNotificationName:notifname object:nil userInfo:[NSDictionary dictionaryWithObject:nsmsg forKey:@"data"] deliverImmediately:YES];
}
