# Building LibreSlicer on Windows
...
Clone the respository. Use a directory relatively close to the drive root, so the path is not too long. Avoid spaces and non-ASCII characters. To place it in `C:\src\LibreSlicer`, run:
```
c:> mkdir src
c:> cd src
...
_As an alternative, you can go to 2.B to compile LibreSlicer using one automatic script._

### Compile the dependencies.
Dependencies are updated seldomly, thus they are compiled out of the LibreSlicer source tree.
Open the MSVC x64 Native Tools Command Prompt and run the following:
```
cd c:\src\LibreSlicer\deps
mkdir build
cd build
cmake ..
cmake --build .
...
### Generate Visual Studio project file for LibreSlicer, referencing the precompiled dependencies.
Open the MSVC x64 Native Tools Command Prompt and run the following:
```
cd c:\src\LibreSlicer\
mkdir build
cd build
cmake .. -DCMAKE_PREFIX_PATH="c:\src\LibreSlicer\deps\build\destdir\usr\local"
```
...
Double-click c:\src\LibreSlicer\build\LibreSlicer.sln to open in Visual Studio and select `LibreSlicer_app_gui` as your startup project (right-click->Set as Startup Project).

Run Build->Rebuild Solution once to populate all required dependency modules. This is NOT done automatically when you Build/Run. If you run both Debug and Release variants, you will need to do this once for each.

Debug->Start Debugging or press F5

LibreSlicer should start. You're up and running!
...
* Change the destination for the dependencies by pointing `-d` to a different directory such as: `build_win.bat -d=s:\LibreSlicerDeps`
