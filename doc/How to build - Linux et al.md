...
This guide describes building LibreSlicer statically against dependencies pulled by our `deps` script. Running all the listed commands in order should result in successful build.
...
LibreSlicer uses CMake and the build is quite simple, the only tricky part is resolution of dependencies. The supported and recommended way is to build the dependencies first and link to them statically. LibreSlicer source base contains a CMake script that automatically downloads and builds the required dependencies. All that is needed is to run the following (from the top of the cloned repository):
...
And that's it. It is now possible to run the freshly built LibreSlicer binary:
...
## Useful CMake flags when building LibreSlicer
...
- `-DSLIC3R_GUI=no` to build the console variant of LibreSlicer
...
As already mentioned above, dynamic linking of dependencies is possible, but LibreSlicer team is unable to troubleshoot (Linux world is way too complex). Feel free to do so, but you are on your own. Several remarks though:
...
At runtime, LibreSlicer needs a way to access its resource files. By default, it looks for a `resources` directory relative to its binary.

If you instead want LibreSlicer installed in a structure according to the File System Hierarchy Standard, use the `SLIC3R_FHS` flag
...
### Desktop Integration (LibreSlicer 2.4 and newer)

If LibreSlicer is to be distributed as an AppImage or a binary blob (.tar.gz and similar), then a desktop integration support is compiled in by default: LibreSlicer will offer to integrate with desktop by manually copying the desktop file and application icon into user's desktop configuration. The built-in desktop integration is also handy on Crosstini (Linux on Chrome OS).

If LibreSlicer is compiled with `SLIC3R_FHS` enabled, then a desktop integration support will not be integrated. One may want to disable desktop integration by running
    
    cmake .. -DSLIC3R_DESKTOP_INTEGRATION=0
    
when building LibreSlicer for flatpack or snap, where the desktop integration is performed by the installer.
