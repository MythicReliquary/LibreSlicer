...
# Building LibreSlicer on Mac OS

To build LibreSlicer on Mac OS, you will need the following software:
...
LibreSlicer comes with a set of CMake scripts to build its dependencies, it lives in the `deps` directory.
Open a terminal window and navigate to LibreSlicer sources directory and then to `deps`.
...
### Building LibreSlicer

If dependencies are built without errors, you can proceed to build LibreSlicer itself.
Go back to top level LibreSlicer sources directory and use these commands:
...
The CMake command above prepares LibreSlicer for building from the command line.
...
and then open the `LibreSlicer.xcodeproj` file.
This should open up XCode where you can perform build using the GUI or perform other tasks.

### Note on Mac OS X SDKs

By default LibreSlicer builds against whichever SDK is the default on the current system.
...
on both the dependencies bundle as well as LibreSlicer itself.

Official macOS LibreSlicer builds are currently (as of LibreSlicer 2.5) built against SDK 10.12 to ensure compatibility with older Macs.
